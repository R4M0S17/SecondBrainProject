...
When that happens, **no `write_file` handler runs**. The UI never shows a TOOL step or Confirm modal; you only see convincing text. This matches smoke under stress: `fs_write_cerebrofiles` got the same parse fallback as calendar (`post_implemefix_smoke.md`).
...
| **P2** | Pre-route file creation (like math fast-path): detect “crea archivo X con Y” → call `write_file` without relying on Qwen3 JSON | See `manual_tests/FIX_TEST2.md` H3.x patterns |
...
So the calendar tool **`get_upcoming_events` / `search_upcoming` may never run** in those turns. This is the same root cause documented in [`FIX_TEST2.md`](FIX_TEST2.md) for Qwen3 sessions.
...
- [`manual_tests/FIX_TEST2.md`](FIX_TEST2.md) — RC-PARSE, tool JSON repair, timeout, intent routing  
- [`manual_tests/post_implemefix_smoke.md`](post_implemefix_smoke.md) — same parse fallback on `fs_write_cerebrofiles` under RAM stress  
- [`manual_tests/test_1.md`](test_1.md) — filesystem authorization and `CEREBRO_FILES_PATH`  
