...
class FleetOrchestrator:
    ...
    def __init__(self) -> None:
        ...
        self._mode: str = "auto"
        self._pinned_model_id: str | None = None
        self.swaps_session_count: int = 0
    ...
    def list_models(self, registry_path: Path | None = None) -> list[ModelConfig]:
        """Return models from the fleet registry with on-disk availability."""
        return load_registry(registry_path)

    def pin_model(self, model_id: str, registry_path: Path | None = None) -> None:
        """Pin a specific model for the session."""
        registry = self.list_models(registry_path)
        match = next((m for m in registry if m.id == model_id), None)
        if match is None:
            raise ValueError(f"Unknown fleet model: {model_id}")
        if not Path(match.path).expanduser().exists():
            raise ValueError(f"Model file not found on disk: {match.path}")
        self._mode = "pinned"
        self._pinned_model_id = model_id
        gpu_layers = match.gpu_layers if match.gpu_layers >= 0 else 0
        self.current_selection = ModelSelection(
            model=match,
            gpu_layers=gpu_layers,
            context_length=match.context_length,
            rationale=f"Pinned by user: {model_id}",
        )

    def use_auto_selection(self, registry_path: Path | None = None) -> ModelSelection | None:
        """Return to automatic model selection."""
        self._mode = "auto"
        self._pinned_model_id = None
        return self.select_on_startup(registry_path=registry_path)
