...
| `inference/providers/` | `llamacpp_provider.py`, `mlx_provider.py` | Chat + embedding provider implementations |
...
- `llamacpp` + `CEREBRO_LLAMACPP_SIMPLE=true`: single llama.cpp server at `CEREBRO_LLAMACPP_URL` (port 8080). Start with `make engine`.
- `llamacpp` (default): `ModelManager` spawns multiple llama.cpp subprocess servers for model-swapping.
- MLX is registered as secondary provider on Apple Silicon (auto-detected).
