...
> **Audience:** the developer implementing the fixes described in `ImplemeFIX.md`.
...
- [x] Save the above as `manual_tests/baseline_before_implemefix.md` so each module can be compared against it.
...
- [x] `manual_tests/post_implemefix_smoke.md` — automated smoke results.
- [x] `scripts/smoke_runner.py` — Module 7 checks (latency, RAM, time, FS, calendar).
- [ ] Open PR — user action.

### 8.5 Smoke report

See [`manual_tests/post_implemefix_smoke.md`](manual_tests/post_implemefix_smoke.md). Re-run: `CEREBRO_SMOKE_QUERY_TIMEOUT=180 make smoke` after `make engine` + `make run`.
...
- [x] Pre-flight baseline captured (§1). **Done 2026-05-19** — see [`manual_tests/baseline_before_implemefix.md`](manual_tests/baseline_before_implemefix.md).
...
- [x] Module 7 — End-to-end smoke green (`make smoke` exit 0; 10 warnings on 8 GB RAM). See `manual_tests/post_implemefix_smoke.md`.
...
**Full report:** [`manual_tests/post_implemefix_smoke.md`](manual_tests/post_implemefix_smoke.md)
...
- [x] Tray UI manual chat session — **2026-05-20** with `lite-8gb.env` + Qwen; see [`pruebas_chat_manual.md`](pruebas_chat_manual.md) and §13.
...
**Source:** [`pruebas_chat_manual.md`](pruebas_chat_manual.md) — 9 prompts, `config/profiles/lite-8gb.env`, `Qwen2.5-Coder-3B-Instruct-Q4_K_M.gguf`, single `make engine` (no embed server).
...
*Comparison of live UI session [`pruebas_chat_manual.md`](pruebas_chat_manual.md) against §11–12 expectations and per-module verification goals.*
