...
| Búsqueda de archivos | Sin fast path / búsqueda básica | **Fast path** + `search_files` mejorado (multi-root, contenido, ES) |
| **Export calendario → archivo** | Solo chat o texto literal en disco | **Fusión** `write_file` con cuerpo de `search_upcoming` (**DONE** 2026-05-25) |
...
**E2E:** `manual_tests/e2e/calendar_birthday_reminder_fix_2026-05-24.md`
...
**Doc:** `manual_tests/e2e/file_write_smart_e2e_2026-05-24.md`
...
**Doc:** `manual_tests/e2e/calendar_next_event_fix_2026-05-24.md`
...
## Búsqueda de archivos (implementado)

| Componente | Cambio |
|------------|--------|
| `core/tools/handlers/filesystem.py` | Búsqueda en todas las rutas autorizadas; omite `.git`/`node_modules`; filtro por nombre y texto dentro del archivo; mensajes en español |
| `core/agents/file_search_fast_path.py` | **NUEVO** — “busca archivos …”, “find files named …” sin pasar por el LLM |
| `core/agents/runtime.py` | Hook fast path en `run()` y `run_streaming()` |

**Ejemplos de prompt:** `busca archivos *.py`, `busca el archivo README.md`, `find files named report`, `busca archivos con extensión md`.

**Tests:** `pytest tests/test_file_search_fast_path.py -q` · live: `python scripts/test_file_search_llamacpp.py`

---

### 9. Export calendario → archivo (fusión) — DONE 2026-05-25

**Problema:** `crea un archivo pruebacalendario.txt con contenido de los proximos 3 cumpleaños…` imprimía la agenda en chat o guardaba solo la frase literal (42 bytes).

**Solución:**

| Componente | Cambio |
|------------|--------|
| `core/agents/file_write_calendar_fusion.py` | **Nuevo** — `fetch_calendar_read_answer` + `try_file_write_calendar_fusion` |
| `core/agents/calendar_fast_path.py` | `fetch_calendar_read_answer()` sin bloqueo de export; chat sigue usando bloqueo |
| `core/agents/runtime.py` | Fusión antes de recordatorios/calendario-chat; no fallback literal |
| `core/agents/reminder_intent_resolver.py` | No clasifica export como `add_reminder` |
| `core/agents/specialized.py` | `write_file` en `CALENDAR_TOOLS` |

**Prompt de prueba:**

```
crea un archivo pruebacalendario.txt con contenido de los proximos 3 cumpleaños en mi calendario
```

**Esperado:** modal `write_file` → archivo en `~/Desktop/CerebroFiles/` con las 3 líneas `- Cumple …`.

**Tests:** `pytest tests/test_file_write_calendar_fusion.py -q` · `python scripts/test_file_write_llamacpp.py`

**E2E:** [`manual_tests/e2e/calendar_file_export_fusion_2026-05-25.md`](../e2e/calendar_file_export_fusion_2026-05-25.md)
...
| 11 | pruebacalendario.txt + 3 cumpleaños | Solo chat / literal en disco | Fusión → archivo con listado real |
...
| Archivo | Contenido |
|---------|-----------|
| `manual_tests/e2e/calendar_birthday_reminder_fix_2026-05-24.md` | Cumpleaños + recordatorios (primera ronda) |
| `manual_tests/e2e/calendar_next_event_fix_2026-05-24.md` | Próximo evento |
| `manual_tests/e2e/file_write_smart_e2e_2026-05-24.md` | Archivos inteligentes |
| `manual_tests/e2e/calendar_file_export_fusion_2026-05-25.md` | **Export calendario → archivo (DONE)** |
| `manual_tests/e2e/calendar_fast_path_e2e_2026-05-24.md` | Fast path calendario (base) |
| `manual_tests/e2e/file_write_fast_path_e2e_2026-05-24.md` | Fast path escritura (base) |
| `manual_tests/sessions/frontend_chat_qwen3_2026-05-21.md` | Sesión anterior |
...
4. Probar prompts de la tabla “Después” y el de export (ítem 11 / sección 9)
5. Para archivos y export calendario: aprobar en el modal de confirmación `write_file`
