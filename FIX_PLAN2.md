# FIX_PLAN2 — Remediation roadmap for `manual_tests/test_1.md`

> **Author role**: Principal Systems Architect & Lead Software Engineer.
> **Companion plan**: this file extends — it does **not** replace — `FIX_CEREBRO.md`. Phase 0 (telemetry) and Phases 1–4 (resource explosion, streaming-bypasses-tools, macOS permissions) are presumed **DONE**; the failures captured in `manual_tests/test_1.md` are residual regressions on top of that baseline.
> **Architectural source of truth**: `cerebro/README.md` → `docs/CLAUDE.md` (root) — module boundaries, port wiring, and the `app_state` injection contract.
...
**Rationale**: Llama‑3.2‑3B at Q4_K_M uses **~80 MB extra** RSS for KV‑cache going from 1.5K → 4K context (with `cache-type-k=q4_0`). Well inside the 5.5 GB ceiling set by `FIX_CEREBRO.md`. The reported `400 Bad Request` on Calendar disappears because the request payload now fits.
...
- The KV cache quantization already in `config/chat.args` (`--cache-type-k q4_0`, `--cache-type-v q4_0`) is exactly the right shape for this hardware. A 4 K context with both K and V quantized to `q4_0` adds only on the order of a few hundred MB to resident memory — well below the 5.5 GB ceiling from `FIX_CEREBRO.md`. Keep these flags. If memory pressure ever returns, the next lever is `--cache-type-k q8_0 --cache-type-v q4_0` (better quality, marginally more RAM), not raising context further.
...
- Runs the ~8 API checks from `manual_tests/test_1.md` / §3 table: health endpoint (after A5), status, a minimal General query, Calendar create (expect `pending_tool` or 200), config PATCH smoke, etc.
...
End-of-plan smoke retest (mirrors `manual_tests/test_1.md` follow-up checklist — automate via **X2** `make smoke`):
...
- **No new Python dependencies.** Every step uses stdlib only (`ast`, `inspect`, `functools.partial`). This preserves the `pyproject.toml` invariant from `FIX_CEREBRO.md`.
...
| Migrating `_tool_node` authorization to `PolicyEngine.validate_call` | Tracked in `FIX_CEREBRO.md` as a follow-up. Step C1 here only unifies the *confirmation* signal, not the full policy pipeline. |
