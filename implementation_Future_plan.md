...
> Phase 0 of the future plan starts only after [`FIX_CEREBRO.md`](FIX_CEREBRO.md) Phase 8 is merged.
...
> **Source documents:** [`READMEA.md`](READMEA.md) (current state) and [`Ideas-futuro.md`](Ideas-futuro.md) (target vision).
...
4. **Log** the step ID + verification stdout into `roadmaps/cognitive-layer-progress.log` (created in Step 0.1).
...
* **Files touched:** `roadmaps/cognitive-layer-progress.log` (new), `roadmaps/PHASES.md` (new).
* **Action:**

```bash
mkdir -p roadmaps
touch roadmaps/cognitive-layer-progress.log
cat > roadmaps/PHASES.md <<'EOF'
...
git add roadmaps/ && git commit -m "phase-0: add cognitive-layer progress scaffolding"
```

* **Verification:**

```bash
test -f roadmaps/cognitive-layer-progress.log && \
test -f roadmaps/PHASES.md && \
...
* **Files touched:** `roadmaps/langchain-audit.md` (new).
* **Action:**
...
```bash
test -f roadmaps/langchain-audit.md && \
...
**Goal:** Replace the "chunks-and-embeddings = memory" model with the **seven memory types** from `Ideas-futuro.md`: Semantic, Episodic, Procedural, Temporal, Project, Tool, Preference. Each lives in the right backing store (LanceDB or SQLite) — never as a separate process.
...
* **Files touched:** `README.md`, `READMEA.md`, `CLAUDE.md`, `ui/tray/src/layouts/Header.tsx`, `ui/tray/index.html`, FastAPI `app = FastAPI(title=...)`.
...
```bash
! grep -rEi "personal assistant" README.md READMEA.md CLAUDE.md ui/tray/index.html ui/tray/src/layouts/Header.tsx
```
...
4. The phase branch is merged (squash) into `main` and `roadmaps/cognitive-layer-progress.log` has the phase's final entry.
...
Anything outside this table requires a written architectural exception logged in `roadmaps/exceptions.md` before installation.

## Appendix C — Failure-handling protocol for Claude Code

* **A test fails after a step:** revert just that step's changes (`git checkout -- <files>`), append the failure to `roadmaps/cognitive-layer-progress.log` as `STEP <id> FAILED`, and stop. Do not start the next step.
...
* **Ambiguity in the spec:** prefer the **smallest, reversible** interpretation and surface the question in `roadmaps/cognitive-layer-progress.log` rather than guessing aggressively.
